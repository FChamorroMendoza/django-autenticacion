...
# **Vistas nuevas para WishList**

# Obtener un WishList por ID
class GetWishListAPIView(RetrieveAPIView):
    '''
    `[METODO GET]`
    Esta vista de API devuelve un solo WishList por su ID.
    '''
    queryset = WishList.objects.all()
    serializer_class = WishListSerializer
    permission_classes = (AllowAny,)  # Permiso público
    lookup_field = 'pk'
...
class PostWishListAPIView(CreateAPIView):
    '''
    `[METODO POST]`
    Esta vista de API nos permite crear un nuevo WishList.
    '''
    queryset = WishList.objects.all()
    serializer_class = WishListSerializer
    permission_classes = (IsAuthenticated,)  # Solo para usuarios autenticados
    authentication_classes = [TokenAuthentication]  # Autenticación con Token

# Actualizar un WishList existente
class UpdateWishListAPIView(UpdateAPIView):
    '''
    `[METODO PUT]`
    Esta vista de API nos permite actualizar un WishList existente.
    Solo se permite para usuarios autenticados o administradores.
    '''
    queryset = WishList.objects.all()
    serializer_class = WishListSerializer
    permission_classes = (IsAuthenticated | IsAdminUser,)  # Permiso para usuarios autenticados o admin
    authentication_classes = [TokenAuthentication]  # Autenticación con Token
    lookup_field = 'pk'
...
class DeleteWishListAPIView(DestroyAPIView):
    '''
    `[METODO DELETE]`
    Esta vista de API elimina un WishList por su ID.
    Solo accesible para administradores.
    '''
    queryset = WishList.objects.all()
    serializer_class = WishListSerializer
    permission_classes = (IsAdminUser,)  # Solo administradores pueden eliminar
    authentication_classes = [TokenAuthentication]  # Autenticación con Token
    lookup_field = 'pk'
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if user is not None:
            # Si el usuario es autenticado, obtenemos o creamos el token
